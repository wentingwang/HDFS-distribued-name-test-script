...
if [ "$1" == "namenode" ]
then
    COMMAND="sudo $HADOOP_HOME/sbin/stop-dfs.sh"
    for node in ${NAME_NODE//,/ }
    do
        echo "Stop name node in $node in $COMMAND"
...
then
    COMMAND="sudo HADOOP_SECURE_DN_USER=$USER $HADOOP_HOME/sbin/hadoop-daemon.sh --config $HADOOP_HOME/etc/hadoop --script hdfs stop datanode;sudo rm -r /data1/datanode"
    for node in ${DATA_NODE//,/ }
    do
        echo "Stop name node in $node in sudo $COMMAND"
        ssh -t -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $node "
        $COMMAND"
    done
else
